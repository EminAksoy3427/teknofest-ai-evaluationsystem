...
CREATE TABLE `__new_analysis_run` (
	...
	CONSTRAINT "analysis_run_stage_check" CHECK("__new_analysis_run"."stage" in ('INGEST_AND_EXTRACT', 'STRUCTURAL_CHECKS', 'SEMANTIC_CHECKS', 'SIMILARITY_CHECKS', 'RUBRIC_EVALUATION')),
...
CREATE UNIQUE INDEX `analysis_run_rubric_version_scope_unique` ON `analysis_run` (`id`,`rubric_version_id`);--> statement-breakpoint
...
CREATE UNIQUE INDEX `criterion_rubric_version_scope_unique` ON `criterion` (`rubric_version_id`,`id`);